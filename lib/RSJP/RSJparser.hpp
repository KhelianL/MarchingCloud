...
inline std::string strtrim(std::string str, std::string chars = " \t\n\r", int max_count = -1, StrTrimDir dirs = STRTRIM_LR)
{
    ...
    { // left trim
        ...
        for (p = 0; p < max_count; ++p)
            if (chars.find(str[p]) == std::string::npos)
                break;
    ...
    { // right trim
        ...
        for (q = 0; q < max_count; ++q)
            if (chars.find(str[strlenm1 - q]) == std::string::npos)
                break;
...
inline std::vector<std::string> split_RSJ_array(const std::string &str)
...
inline RSJresource::~RSJresource()
{
    if (parsed_data_p)
        delete parsed_data_p;
}

inline RSJresource::RSJresource(const RSJresource &r)
{
    data = r.data;
    _exists = r._exists;
    if (r.parsed_data_p)
...
inline RSJresource &RSJresource::operator=(const RSJresource &r)
{
    data = r.data;
    _exists = r._exists;
    if (r.parsed_data_p)
...
inline int RSJresource::size(void)
{
    if (!exists())
        return (0);
...
inline RSJresourceType RSJresource::type(void)
{
    if (!exists())
        return (RSJ_UNINITIATED);
    parse(); // parse if not parsed
...
inline std::string RSJresource::as_str(bool print_comments, bool update_data)
{
    if (exists())
    {
        std::string ret;
        ...
        if (parsed_data_p->type == RSJ_OBJECT)
        {
            ...
            for (auto it = parsed_data_p->object.begin(); it != parsed_data_p->object.end(); ++it)
            {
                ...
                if (std::next(it) != parsed_data_p->object.end())
                    ret += ",";
                if (print_comments)
        ...
        else if (parsed_data_p->type == RSJ_ARRAY)
        {
            ...
            for (auto it = parsed_data_p->array.begin(); it != parsed_data_p->array.end(); ++it)
            {
                ...
                if (std::next(it) != parsed_data_p->array.end())
                    ret += ",";
                if (print_comments)
...
inline RSJresourceType RSJresource::parse(bool force)
{
    if (!parsed_data_p)
        parsed_data_p = new RSJparsedData;
    if (parsed_data_p->type == RSJ_UNKNOWN || force)
        parsed_data_p->parse(data, RSJ_UNKNOWN);
    return (parsed_data_p->type);
}

inline void RSJresource::parse_full(bool force, int max_depth, int *parse_count_for_verbose_p)
...
inline void RSJresource::fast_parse(std::string *str_p, bool copy_string, int max_depth, int *parse_start_str_pos)
{
...
inline RSJobject &RSJresource::as_object(bool force)
{
    if (!parsed_data_p)
        parsed_data_p = new RSJparsedData;
    if (parsed_data_p->type == RSJ_UNKNOWN || force)
        parsed_data_p->parse(data, RSJ_OBJECT);
    return (parsed_data_p->object);
}

inline RSJresource &RSJresource::operator[](std::string key)
{                                // returns reference
...
inline RSJarray &RSJresource::as_array(bool force)
{
    if (!parsed_data_p)
        parsed_data_p = new RSJparsedData;
    if (parsed_data_p->type == RSJ_UNKNOWN || force)
        parsed_data_p->parse(data, RSJ_ARRAY);
    return (parsed_data_p->array);
}

inline RSJresource &RSJresource::operator[](int indx)
{ // returns reference
    as_array();
...
inline vectorType RSJresource::as_vector(const vectorType &def)
...
inline mapType RSJresource::as_map(const mapType &def)
{ // returns copy -- for being consistent with other 'as' specializations
    if (!exists())
        return (def);
    mapType ret;
...
// RSJobject
template <>
inline RSJobject RSJresource::as<RSJobject>(const RSJobject &def)
{ // returns copy -- for being consistent with other 'as' specializations
    if (!exists())
        return (def);
    return (as_object());
...
// RSJarray
template <>
inline RSJarray RSJresource::as<RSJarray>(const RSJarray &def)
{ // returns copy -- for being consistent with other 'as' specializations
    if (!exists())
        return (def);
    return (as_array());
...
inline bool RSJresource::as<bool>(const bool &def)
{
    ...
    if (cleanData == "true" || cleanData == "TRUE" || cleanData == "True" || atoi(cleanData.c_str()) != 0)
        return (true);
    return (false);
}
