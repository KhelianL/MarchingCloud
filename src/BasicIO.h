...
namespace OFFIO
{
    template <class point_t, class tri_t>
    bool openTriMesh(const std::string &filename,
    ...
    bool save(const std::string &filename, std::vector<point_t> &vertices, std::vector<face_t> &faces)
    {
        std::ofstream myfile;
        myfile.open(filename.c_str());
        if (!myfile.is_open())
...
namespace OBJIO
{
    ...
    template <class point_t, typename face_t>
    bool save(
